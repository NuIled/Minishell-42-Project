...
/*   Updated: 2023/06/25 19:46:47 by srachdi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *word(char *line,int *n)
{
	...
	{
		if(is_q(line[i]))
			i = in_q(line,line[i],i + 1); //get position of next quote
		i++;/*go after the quote or the chars*/
	}
...
int op_index(char *str)
{
	...
	{   
		if(is_q(str[i]))
			i = in_q(str, str[i], i + 1);/*skip chars inside quotes*/
		if(is_op(str[i]))
			return i;
		i++;
	}
